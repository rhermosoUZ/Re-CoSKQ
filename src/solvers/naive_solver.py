...
class NaiveSolver(Solver):
    def __init__(self, query: KeywordCoordinate, data: dataset_type, cost_function: CostFunction, result_length=10):
        ...
        logger.debug('creating with query {}, data {}, cost function {} and result length {}'.format(query, dataset_comprehension(data), cost_function, result_length))
        super().__init__(query, data, cost_function, result_length)
    ...
    def solve(self) -> typing.List[solution_type]:
        ...
        logger.debug('solved for {} with length {}'.format(result_list_comprehension(result_list), self.result_length))
        return result_list
