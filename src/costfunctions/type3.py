...
class Type3(CostFunction):
    ...
    def solve(self, query: KeywordCoordinate, dataset: dataset_type) -> float:
        ...
        solution = self.alpha * self.get_minimum_for_query(query, dataset) + self.beta * self.get_maximum_for_dataset(dataset) + self.omega * self.get_maximum_keyword_distance(query, dataset)
